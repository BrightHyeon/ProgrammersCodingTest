...
주어진 input값 범위가 2의 20승이면 매우 큰 숫자이다. 규칙을 찾는 문제라고 1차적으로 파악.
처음에는 이분법적으로 접근하려하였으나, 문제를 잘 읽고 규칙을 찾다보니 A와 B가 대결하는 시점에서는 두 숫자가 같아지는 것을 파악.
while문을 돌리며 같아지는 시점을 찾아서 return.
