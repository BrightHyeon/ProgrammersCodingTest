...
swift의 배열 contains 메서드는 O(n)의 시간복잡도를 나타내기에, 이 문제에선 효율이 좋지않다고 생각했다.
이 문제에선 끝말잇기 틀린 것 이외에 중복단어 사용을 체크해야하는데, 중복단어 체크는 단순히 Set을 사용하고, count의 변화만 감지하면되기 때문에 Set을 사용해서 효율을 증가시켰다.
